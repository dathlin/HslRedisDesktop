...
namespace HslRedisDesktop
{
	public partial class SetValueControl : UserControl
	{
		...
		private void RefreshKey( )
		{
			...
			OperateResult<string[]> read = redisClient.SetMembers( stringKeyName );
			if (!read.IsSuccess)
			{
		...
		public void SetNewKey( RedisClient redisClient, string key )
		{
			...
			keyOperateControl1.SetRedisClient( redisClient, key );
			RefreshKey( );
		}

		
		private string stringKeyName = string.Empty;                // 字符串键名
		private RedisClient redisClient = null;                     // 当前的连接
